...
ipcMainInternal.on('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_OPEN', (event, url, frameName, features) => {
  ...
  if (event.sender.getLastWebPreferences().nativeWindowOpen || event.sender.getLastWebPreferences().sandbox) { throw new Error('GUEST_WINDOW_MANAGER_WINDOW_OPEN denied: expected native window.open'); }
